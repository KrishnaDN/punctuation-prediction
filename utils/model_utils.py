...
def average_models(src_path, dst_model, val_best=True, num=2, min_epoch=0, max_epoch=10000):
    checkpoints = []
    val_scores = []
    if val_best:
        ...
        for y in yamls:
            with open(y, 'r') as f:
                ...
                f1_score = dic_yaml['f1-score']
                epoch = dic_yaml['epoch']
                if epoch >= min_epoch and epoch <= max_epoch:
                    val_scores += [[epoch, f1_score]]
        print(val_scores)
        val_scores = np.array(val_scores)
        sort_idx = np.argsort(val_scores[:, -1])[::-1]
